...
> **前提**: ローカルはすでに対象PRのブランチにcheckout済みであること。
...
- **スレッドの深掘り** — 未解決（resolved: false）のスレッドや、提案に対する著者の返答がある場合はそのやり取りを踏まえてさらなる観点を示す
...
checkout済みのため、diffで変更箇所を把握した後の深掘りはローカルファイルを直接参照する。
