...
**原則**: 「何を作るか」は所与、決めるのは「Rustとしてどう書くか」だけ。要件そのものへの疑義・改善提案・スコープ外の修正（ついでのリファクタ等）はしない。目的レベル（何を達成すべきか）が曖昧・矛盾・欠落している場合のみ `AskUserQuestion` で確認し、外部から見た振る舞いに影響しない技術選択（内部エラー型の形、関数分割、命名の細部など）は Rust として最も自然な形を自分で選ぶ。区別に迷ったら「振る舞いが変わるか」で判定する。実装中に気づいた既存コードの問題は直さず「要確認」に書く（対応は `/rewrite` `/brushup` `/refine` の領分）。
